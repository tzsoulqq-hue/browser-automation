...
- 公开浏览器自动化契约来自 `contracts/browserautomation`；runtime 私有配置、provider raw metadata 和内部执行状态留在本仓内部 proto。
- session/task/artifact/lease 等对外模型以公开 proto 为源头，应用层直接使用生成类型，避免手写重复模型和大面积字段映射。
- 内部业务命令、状态、runtime 配置、artifact 引用、事件和 raw metadata 也优先使用本仓内部 proto 建模；不要只用 Go struct 作为私有契约源头。
- 浏览器 session 必须持久化并使用 lease 管理；执行 task 前先获取有效 lease，持久化 adapter 必须用原子更新维护 lease token、owner 和过期时间。
